<template>
  ...
  <nav class="tp-tabs" role="tablist">
    <button
      class="tp-tab" role="tab"
      :class="{ 'tp-tab--active': activeTab === 'classifica' }"
      @click="activeTab = 'classifica'"
      :aria-selected="activeTab === 'classifica'"
    >
    ...
    <button
      class="tp-tab" role="tab"
      :class="{ 'tp-tab--active': activeTab === 'punti' }"
      @click="activeTab = 'punti'"
      :aria-selected="activeTab === 'punti'"
    >
    ...
    <button
      class="tp-tab" role="tab"
      :class="{ 'tp-tab--active': activeTab === 'storico' }"
      @click="activeTab = 'storico'"
      :aria-selected="activeTab === 'storico'"
    >
  ...
  <div v-show="activeTab === 'classifica'" class="tp-panel" role="tabpanel">
    ...
    <template v-else>
      ...
      <div class="stats-row">
        ...
        <div class="stat-card">
          <div class="stat-val">{{ sortedHistory.length }}</div>
          <div class="stat-label">Assegnazioni</div>
        </div>
        <div class="stat-card">
          <div class="stat-val">{{ leadGap }}</div>
          <div class="stat-label">Vantaggio 1°</div>
        </div>
      </div>
    </template>
  </div>
  ...
  <div v-show="activeTab === 'punti'" class="tp-panel" role="tabpanel">
    <div class="form-card">
      ...
      <div class="fc-group">
        <div class="fc-label">
          <span class="fc-step">1</span> Squadra
        </div>
      ...
      <div class="fc-group">
        <div class="fc-label">
          <span class="fc-step">2</span> Tipo operazione
        </div>
      ...
      <div class="fc-group">
        <div class="fc-label">
          <span class="fc-step">3</span> Punti
        </div>
        <div class="pts-quick">
        ...
        <div class="pts-wrap">
          <button type="button" class="pts-btn pts-minus" @click="pointsToAdd = Math.max(0, pointsToAdd - 1)" aria-label="Diminuisci">−</button>
          <input
            class="pts-input" type="number"
            v-model.number="pointsToAdd"
            min="0" inputmode="numeric" placeholder="0"
          />
          <button type="button" class="pts-btn pts-plus" @click="pointsToAdd++" aria-label="Aumenta">+</button>
        </div>
      </div>
  ...
  <div v-show="activeTab === 'storico'" class="tp-panel" role="tabpanel">
    ...
    <div class="h-filters">
      <button
        class="h-filter-btn"
        :class="{ 'h-filter-btn--active': historyFilter === '' }"
        @click="historyFilter = ''"
      >Tutti</button>
      <button
        v-for="t in teams" :key="t.id"
        class="h-filter-btn"
        :class="[t.id + '-filter', { 'h-filter-btn--active': historyFilter === t.id }]"
        @click="historyFilter = t.id"
      >{{ t.name }}</button>
    </div>
    ...
    <div class="h-list">
      <transition-group name="h-item">
        <div
          v-for="(entry, index) in filteredHistory.slice(0, visibleHistory)"
          :key="entry.timestamp + index"
          class="h-entry"
          :class="{ 'h-entry--recent': index < 3 && historyFilter === '' }"
        >
          ...
          <button
            ...
            class="h-del" aria-label="Elimina voce"
...
.tp-title  { font-size: 1.05rem; font-weight: 900; color: #fff; line-height: 1.2; }
...
.tp-username { color: #fff; font-size: .82rem; font-weight: 700; }
.tp-logout {
  background: rgba(220,53,69,.15); border: 1.5px solid rgba(220,53,69,.4);
...
.tp-tab {
  ...
  padding: 0.85rem 0.5rem; font-size: 0.88rem; font-weight: 800;
...
.skeleton-row  { display: flex; align-items: center; gap: 0.75rem; }
@keyframes shimmer {
...
.sk-bar    { flex: 1; height: 24px; }
.sk-num    { width: 48px; height: 24px; flex-shrink: 0; }
...
.lock-box h3 { font-size: 1.1rem; font-weight: 800; color: #555; margin-bottom: 0.4rem; }

/* ── FIRST PLACE CARD ── */
.fp-card {
  display: flex; align-items: center; justify-content: space-between;
  padding: 1rem 1.25rem; border-radius: 18px; margin-bottom: 1rem;
  border: 2px solid transparent;
}
.fp-left   { display: flex; align-items: center; gap: 0.75rem; }
.fp-medal  { font-size: 2.5rem; filter: drop-shadow(0 2px 6px rgba(0,0,0,0.15)); }
.fp-name   { font-size: 1.3rem; font-weight: 900; line-height: 1.1; }
.fp-label  { font-size: 0.72rem; color: #888; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; }
.fp-score  { font-size: 2rem; font-weight: 900; }
.fp-score small { font-size: 0.75rem; font-weight: 700; opacity: 0.6; }

.rossi-fp     { background: linear-gradient(135deg, #fff0f1, #ffe0e3); border-color: rgba(220,53,69,0.25); }
.verdi-fp     { background: linear-gradient(135deg, #f0fbf3, #d8f5e1); border-color: rgba(40,167,69,0.25); }
.arancioni-fp { background: linear-gradient(135deg, #fff8f0, #ffe8d0); border-color: rgba(253,126,20,0.25); }
.blu-fp       { background: linear-gradient(135deg, #f0f8ff, #d5ecff); border-color: rgba(0,123,255,0.25); }
.fucsia-fp    { background: linear-gradient(135deg, #fff0f8, #ffd5ec); border-color: rgba(232,62,140,0.25); }
.gialli-fp    { background: linear-gradient(135deg, #fffde0, #fff5b0); border-color: rgba(200,150,12,0.25); }

/* ── RANK LIST ── */
.rank-list { display: flex; flex-direction: column; gap: 0.55rem; margin-bottom: 1.2rem; }
.rank-row {
  display: flex; align-items: center; gap: 0.75rem;
  padding: 0.75rem 1rem; border-radius: 14px;
  background: #f9f9ff; border: 1.5px solid rgba(0,0,0,0.05);
  transition: transform .15s;
}
.rank-row--podium { background: #f4f5ff; }
.rank-row:active { transform: scale(0.99); }
.rank-pos { width: 32px; text-align: center; font-size: 1.15rem; flex-shrink: 0; }
.rank-num { font-size: 0.8rem; font-weight: 800; color: #bbb; }
.rank-info { flex: 1; min-width: 0; }
.rank-name { font-size: 0.95rem; font-weight: 800; margin-bottom: 0.3rem; }
.rank-bar-track { height: 7px; background: #eee; border-radius: 99px; overflow: hidden; }
.rank-bar { height: 100%; border-radius: 99px; transition: width 0.8s cubic-bezier(.16,1,.3,1); min-width: 4%; }
.rank-score { font-size: 1rem; font-weight: 900; flex-shrink: 0; text-align: right; min-width: 50px; }
.rank-score-unit { font-size: 0.65rem; font-weight: 700; opacity: 0.5; }

/* Team colors */
.rossi-text     { color: #DC3545; } .verdi-text   { color: #28A745; }
.arancioni-text { color: #FD7E14; } .blu-text     { color: #007BFF; }
.fucsia-text    { color: #E83E8C; } .gialli-text  { color: #a07800; }
...
.rossi-dot     { background: #DC3545; } .verdi-dot   { background: #28A745; }
.arancioni-dot { background: #FD7E14; } .blu-dot     { background: #007BFF; }
.fucsia-dot    { background: #E83E8C; } .gialli-dot  { background: #c8960c; }

/* ── STATS ROW ── */
.stats-row { display: flex; gap: 0.55rem; margin-top: 1rem; }
.stat-card {
  flex: 1; text-align: center; padding: 0.9rem 0.5rem;
  background: #f4f5ff; border-radius: 14px;
  border: 1.5px solid rgba(102,126,234,0.15);
}
.stat-val   { font-size: 1.4rem; font-weight: 900; color: #667eea; line-height: 1; }
.stat-label { font-size: 0.68rem; font-weight: 700; color: #aaa; text-transform: uppercase; letter-spacing: 0.06em; margin-top: 0.2rem; }
...
.fc-group { display: flex; flex-direction: column; gap: 0.6rem; }
.fc-label {
...
.team-pill {
  ...
  border: 2px solid transparent; font-size: 0.88rem; font-weight: 800;
  cursor: pointer; font-family: 'Nunito', sans-serif;
  transition: all .15s; -webkit-tap-highlight-color: transparent; min-height: 40px;
}
.tp-pill-dot { width: 8px; height: 8px; border-radius: 50%; background: currentColor; opacity: 0.6; flex-shrink: 0; }
...
.rossi-pill.team-pill--active     { background: #ffe0e3; border-color: #DC3545; box-shadow: 0 3px 12px rgba(220,53,69,.25); transform: translateY(-1px); }
.verdi-pill.team-pill--active     { background: #d8f5e1; border-color: #28A745; box-shadow: 0 3px 12px rgba(40,167,69,.25); transform: translateY(-1px); }
.arancioni-pill.team-pill--active { background: #ffe8d0; border-color: #FD7E14; box-shadow: 0 3px 12px rgba(253,126,20,.25); transform: translateY(-1px); }
.blu-pill.team-pill--active       { background: #d5ecff; border-color: #007BFF; box-shadow: 0 3px 12px rgba(0,123,255,.25); transform: translateY(-1px); }
.fucsia-pill.team-pill--active    { background: #ffd5ec; border-color: #E83E8C; box-shadow: 0 3px 12px rgba(232,62,140,.25); transform: translateY(-1px); }
.gialli-pill.team-pill--active    { background: #fff5b0; border-color: #c8960c; box-shadow: 0 3px 12px rgba(200,150,12,.25); transform: translateY(-1px); }

.game-pills { display: flex; gap: 0.5rem; }
.game-pill {
  flex: 1; display: flex; align-items: center; justify-content: center; gap: 0.4rem;
  padding: 0.65rem; border-radius: 14px;
  border: 2px solid rgba(102,126,234,.18); background: #f4f5ff; color: #667eea;
...
.game-pill--active    { background: #667eea; color: #fff; border-color: #667eea; box-shadow: 0 4px 14px rgba(102,126,234,.35); }
.game-pill--penalty   { border-color: rgba(220,53,69,.2); background: #fff0f1; color: #DC3545; }
.game-pill--penalty.game-pill--active { background: #DC3545; color: #fff; border-color: #DC3545; box-shadow: 0 4px 14px rgba(220,53,69,.35); }

.pts-quick { display: flex; gap: 0.4rem; }
.pts-q {
  flex: 1; padding: 0.55rem 0; border-radius: 12px;
  border: 2px solid rgba(102,126,234,.18); background: #f4f5ff; color: #667eea;
...
.pts-input {
  flex: 1; text-align: center; font-size: 1.6rem; font-weight: 900;
  border: 2px solid #e0e0e0; border-radius: 14px;
  padding: 0.6rem; outline: none; min-height: 54px;
  transition: border-color .2s; font-family: 'Nunito', sans-serif;
...
.pts-preview {
  background: #f4f5ff; border-radius: 12px;
  padding: 0.7rem 1rem; font-size: 0.9rem; color: #555; font-weight: 600;
  text-align: center;
}
.pos-pts { color: #28A745; font-weight: 900; }
.neg-pts { color: #DC3545; font-weight: 900; }
...
.tp-submit {
  ...
  box-shadow: 0 6px 22px rgba(102,126,234,.42);
...
.h-count { font-size: 0.78rem; font-weight: 700; color: #bbb; margin-bottom: 0.5rem; text-transform: uppercase; letter-spacing: 0.06em; }
.h-list  { display: flex; flex-direction: column; gap: 0.45rem; }
.h-entry {
  display: flex; align-items: center; gap: 0.6rem;
  padding: 0.75rem 0.9rem; border-radius: 14px;
  background: #f9f9ff; border: 1.5px solid rgba(0,0,0,.05);
  transition: all .15s;
}
...
.h-bottom { display: flex; align-items: center; gap: 0.3rem; margin-top: 0.15rem; }
.h-team { font-size: 0.88rem; font-weight: 800; }
.h-game { font-size: 0.82rem; font-weight: 700; color: #555; }
.h-user { font-size: 0.72rem; color: #aaa; font-weight: 600; }
.h-user i { margin-right: 0.2rem; }
.h-date { font-size: 0.72rem; color: #aaa; font-weight: 600; }
.h-sep  { font-size: 0.7rem; color: #ccc; }
.h-pts { font-size: 1rem; font-weight: 900; flex-shrink: 0; min-width: 44px; text-align: right; }
...
.h-loadmore {
  ...
  background: none; color: #aaa; font-size: 0.88rem; font-weight: 700;
  cursor: pointer; font-family: 'Nunito', sans-serif;
  -webkit-tap-highlight-color: transparent; min-height: 48px;
}
...
.modal-bg {
  ...
  display: flex; align-items: center; justify-content: center;
  z-index: 9999; backdrop-filter: blur(6px); padding: 1.25rem;
}
.modal-box {
...
.modal-head {
  padding: 1rem 1.25rem; background: #fff8f8;
  border-bottom: 1px solid rgba(220,53,69,.15);
  font-size: 0.95rem; font-weight: 800; color: #c0392b;
  display: flex; align-items: center; gap: 0.5rem;
}
.modal-x { margin-left: auto; background: none; border: none; font-size: 1.4rem; cursor: pointer; opacity: 0.7; color: #555; }
.modal-body { padding: 1.25rem; }
.modal-details { display: flex; flex-direction: column; gap: 0.5rem; margin-bottom: 1rem; }
.md-row {
  display: flex; justify-content: space-between; align-items: center;
  padding: 0.5rem 0.75rem; background: #f8f9fa; border-radius: 10px;
  font-size: 0.88rem;
}
.md-row span { color: #888; font-weight: 600; }
.md-row strong { font-weight: 800; }
.modal-warn {
  padding: 0.7rem 0.9rem; background: #fff3cd;
  border-radius: 10px; border: 1px solid #ffc107;
  font-size: 0.82rem; font-weight: 700; color: #856404;
}
.modal-foot {
...
.modal-del {
  ...
  cursor: pointer; font-family: 'Nunito', sans-serif;
  box-shadow: 0 4px 14px rgba(220,53,69,.35); transition: transform .12s;
}
.modal-del:active { transform: scale(0.97); }
.modal-del:disabled { opacity: 0.55; cursor: not-allowed; }
.modal-cancel {
  padding: 0.8rem 1.2rem; background: #f0f0f0;
  border: none; border-radius: 12px;
  color: #555; font-size: 0.92rem; font-weight: 800;
...
@media (prefers-color-scheme: dark) {
  ...
  .md-row span   { color: #666; }
  .md-row strong { color: #eee; }
